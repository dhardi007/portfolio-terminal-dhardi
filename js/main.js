...
const certFrames = ['◴', '◷', '◶', '◵'];
let certIdx = 0;
function spinCert(elId) {
  const el = document.getElementById(elId);
  if (el) el.textContent = certFrames[certIdx++ % certFrames.length];
}
setInterval(() => spinCert('cert-spinner'), 400);
setInterval(() => spinCert('cert-modal-spinner'), 400);
function advanceBar(barId) {
  const el = document.getElementById(barId);
  if (!el) return;
  const w = parseInt(el.style.width) || 42;
  el.style.width = Math.min(99, w + (Math.random() > 0.7 ? 1 : 0)) + '%';
}
setInterval(() => advanceBar('cert-bar'), 3000);
setInterval(() => advanceBar('cert-modal-bar'), 3000);
